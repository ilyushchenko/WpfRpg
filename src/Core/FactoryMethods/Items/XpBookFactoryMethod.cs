...
{
    internal class CXpBookFactoryMethod : CItemFactoryMethod
    {
        public override IInventoryItem Create(XElement itemData)
        {
            ...
            var xpBook = new CXpBook(id, name, cost, 0, description);
            return xpBook;
        }

        public override IInventoryItem CreateFromJson(String itemJsonData)
        {
            return JsonConvert.DeserializeObject<CXpBook>(itemJsonData);
        }
    }
}